...
**#2026-02-14**
1. **重构音视频录像架构，实现 1080P 30FPS 高清丝滑录像**
   - **遇到的问题**：
...
2. **解决系统启动时序冲突**
   - **遇到的问题**：主程序启动瞬间由于 `mjpg-streamer` 未初始化完成，导致 `Connection refused` 报错退出。
   - **解决方案**：
     - **延迟启动**：在 Makefile 启动链中增加 `sleep 3` 缓冲。
     - **自动重连**：在 C++ `CameraManager::open()` 中增加循环重试机制，增强系统健壮性。
